...
void main() {
  group('RestaurantListProvider Unit Tests', () {
    ...
    test(
      '2. Should return ResultStateHasData when API call is successful',
      () async {
        final mockApiService = MockApiService();
        ...
        expect(
          provider.state,
          isA<ResultStateHasData<RestaurantListResponse>>(),
        );
        final stateData =
            provider.state as ResultStateHasData<RestaurantListResponse>;
    ...
    test('3. Should return ResultStateError when API call fails', () async {
      ...
      when(
        mockApiService.getRestaurantList(),
      ).thenThrow(Exception('Failed to connect to network'));
      final provider = RestaurantListProvider(apiService: mockApiService);
